...
import type { IWorkflowEngine, WorkflowSpec } from '@kb-labs/workflow-contracts';
import type {
...
export class JobBroker {
        ...
        jobs: {
          execute: {
            ...
            steps: [
              {
                name: 'run',
                uses: `plugin:${this.callerManifest.id}/${request.handler}`,
                with: request.input as Record<string, unknown> | undefined,
   ...
   * List jobs
   */
  async list(_filter?: JobFilter): Promise<JobInfo[]> {
    try {
      // TODO: Implement proper filtering once workflow-engine supports it
      if (!this.workflowEngine.listRuns) {
        return [];
      }

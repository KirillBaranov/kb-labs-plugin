...
import type { WorkflowRun, IWorkflowEngine } from '@kb-labs/workflow-contracts';
import type {
  JobHandle,
  JobStatus,
  JobResult,
...
export class JobHandleImpl<T = unknown> implements JobHandle<T> {
  ...
  private buildResult(run: WorkflowRun): JobResult<T> {
    if (run.status === 'success') {
      // Extract outputs from first job's first step
      ...
      return {
        ...
        error: stepError
          ? {
              code: stepError.code || 'UNKNOWN_ERROR',
              message: stepError.message || 'Job failed',

/**
 * @module @kb-labs/plugin-runtime-v3/__tests__/plugin-api
 *
...
import type { PermissionSpec, CacheAdapter, Logger } from '@kb-labs/plugin-contracts-v3';
...
describe('Plugin API', () => {
  ...
  describe('output API', () => {
    it('should store and retrieve result', () => {
      ...
      api.output.result({ status: 'success', count: 42 });

      const result = api.output.getResult<{ status: string; count: number }>();
    ...
    it('should store and retrieve metadata', () => {
      ...
      api.output.meta('duration', 1234);
      api.output.meta('itemsProcessed', 100);
      api.output.meta('warnings', ['warn1', 'warn2']);

      const meta = api.output.getMeta();
      expect(meta).toEqual({
        duration: 1234,
        itemsProcessed: 100,
    ...
    it('should return copy of metadata (immutability)', () => {
      ...
      api.output.meta('foo', 'bar');
      const meta1 = api.output.getMeta();
      meta1.mutated = 'should not affect original';

      const meta2 = api.output.getMeta();
      expect(meta2).toEqual({ foo: 'bar' });
      expect(meta2.mutated).toBeUndefined();
    });
  });
